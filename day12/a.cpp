    ...
    while(!q.empty() &&  notFound) {
        for(int i = 0; i < q.size(); i++ ){
            ...
            for(auto p : dirs) {
                int nr = x + p.first;
                int nc = y + p.second;
                if(checkBounds(nr, nc) && m[nr][nc] == 'E' && checkTransition(nodeval, 'z')) {
                    moves = min(moves, dist+1);
                    notFound = false;
                }
                else if(checkBounds(nr, nc) && m[nr][nc] == 'E') continue;
                else if(checkBounds(nr, nc) && m[nr][nc] != '*' && checkTransition(nodeval, m[nr][nc])) {
